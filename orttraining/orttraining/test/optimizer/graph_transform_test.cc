...
std::vector<std::vector<int64_t>> input_dims) {
  const PathString model_uri = ORT_TSTR(model_path + ".onnx");
  ...
  for (auto i = 0; i < total_rank; i++) {
    ...
    auto model_uri2 = ORT_TSTR(model_path + "_partition_rank_" + std::to_string(i) + ".onnx");
  ...
  ORT_ENFORCE(ret.IsOK());
  auto model_uri2 = ORT_TSTR(model_path + "_partition_combine.onnx");
